...
const FeaturedProducts = () => {
  ...
  return (
    <section
      ...
      ref={sectionRef}
      style={{
        backgroundImage: `linear-gradient(rgba(255,247,237,0.85),rgba(255,247,237,0.95)), url(${fondoProductos})`,
    ...
    >
      <div className="max-w-7xl mx-auto">
        <div className={`text-center mb-20 transition-all duration-[1800ms] ease-in-out ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          ...
          <h2 className="text-5xl lg:text-6xl font-light text-orange-950 mb-4 leading-tight">
            Piezas <span className="font-serif italic text-amber-900">excepcionales</span>
          </h2>
          ...
          <p className="text-xl text-orange-700 max-w-3xl mx-auto leading-relaxed font-light">
            Piezas excepcionales, hechas con <span className="font-semibold text-amber-900">pasión</span>, <span className="font-semibold text-amber-900">dedicación</span> y el arte del cuero argentino.
          </p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
          ...
          {placeholders.map((_, i) => (
            <div
              key={`ph-${i}`}
              className={`bg-orange-100 rounded-2xl aspect-square transition-all duration-[1800ms] ease-in-out ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}
